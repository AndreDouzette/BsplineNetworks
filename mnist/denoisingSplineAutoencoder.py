...
runNumber = len(glob.glob("./data/splineDAE{}-{}*".format(internalKnots, p)))
print(runNumber)

if not os.path.exists("./data/splineDAE{}-{}-{}".format(internalKnots, p, runNumber)):
	os.makedirs("./data/splineDAE{}-{}-{}".format(internalKnots, p, runNumber))
...
encoder1 = spline(tf.matmul(corrupt(target1), W1) + b1, t1, C1, p)
...
with tf.Session() as sess:
	...
	saver.save(sess, "./data/splineDAE{}-{}-{}/firstLayer.ckpt".format(internalKnots, p, runNumber))
	...
	for e in range(encoderEpochs):
		...
		if(e%displayStep == 0):
				...
				print("{:5d}: {:7.7f}".format(e, loss))
	saver.save(sess, "./data/splineDAE{}-{}-{}/secondLayer.ckpt".format(internalKnots, p, runNumber))
	...
	saver.save(sess, "./data/splineDAE{}-{}-{}/fineTuning.ckpt".format(internalKnots, p, runNumber))
	print("duration: {}".format(time.time() - time0))
	np.save("./data/splineDAE{}-{}-{}/error".format(internalKnots, p, runNumber), [epochArray, trainAccuracyArray, testAccuracyArray])
